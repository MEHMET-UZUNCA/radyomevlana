...
@section('title', 'SEO Düzenle – ' . $pageSeo->path)

@section('content')
<div class="max-w-2xl">
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
        <form action="{{ route('admin.seo.update', $pageSeo) }}" method="POST" class="space-y-4">
            @csrf @method('PUT')

            <div>
                <label class="text-xs text-gray-500 mb-1.5 block font-medium">Sayfa Yolu *</label>
                <input type="text" name="path" required value="{{ old('path', $pageSeo->path) }}"
            ...
            <div>
                <label class="text-xs text-gray-500 mb-1.5 block font-medium">Sayfa Başlığı</label>
                <input type="text" name="title" value="{{ old('title', $pageSeo->title) }}" maxlength="120"
            ...
            <div>
                ...
                <textarea name="description" rows="3" maxlength="320"
                    class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-brand resize-none">{{ old('description', $pageSeo->description) }}</textarea>
            </div>

            <div>
                <label class="text-xs text-gray-500 mb-1.5 block font-medium">Anahtar Kelimeler</label>
                <input type="text" name="keywords" value="{{ old('keywords', $pageSeo->keywords) }}"
                    class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-brand">
            </div>

            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label class="text-xs text-gray-500 mb-1.5 block font-medium">OG Başlık</label>
                    <input type="text" name="og_title" value="{{ old('og_title', $pageSeo->og_title) }}" maxlength="120"
                        class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-brand">
                </div>
                <div>
                    <label class="text-xs text-gray-500 mb-1.5 block font-medium">OG Görsel URL</label>
                    <input type="url" name="og_image" value="{{ old('og_image', $pageSeo->og_image) }}"
                        class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-brand">
                </div>
            </div>
